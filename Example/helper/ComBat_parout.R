...
ComBatp <- function(dat, batch, mod=NULL, par.prior=TRUE,prior.plots=FALSE,mean.only=FALSE,ref.batch=NULL) {
  ...
  ## experimenting the adjustment standard error
  ## prop <- table(batch)/length(batch)
  ## result <- apply(delta.star,2,function(x) sum(x*prop))
  ## var.adjusted <- var.pooled/result
  sres <- (bayesdata*(sqrt(var.pooled)%*%t(rep(1,n.array))))
  ## sres <- (bayesdata*(sqrt(var.adjusted)%*%t(rep(1,n.array))))
  bayesdata <- sres+stand.mean
  ...
  return(list(adjdata=bayesdata,
              res=sres,
              alpha_hat=grand.mean,  # background expression alpha of genes
              beta_hat=B.hat[(n.batch+1):nrow(B.hat),],  # condition effect, beta_g 
              sigma_hat=var.pooled,  # background variance sigma (variance of error term espilon)
              gamma_hat=gamma.hat, delta_hat=delta.hat,  # estimated batch parameters before EB adjustment, matrix with dimension #batch * #genes
              gamma_star=gamma.star, delta_star=delta.star))  # posterior batch parameters after EB shrinkage, matrix with dimension #batch * #genes
}
